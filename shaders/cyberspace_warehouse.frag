...
// Converted to Flutter by Claude
//
// Changes from original Shadertoy:
// 1. Added #include <flutter/runtime_effect.glsl>
// 2. iResolution (vec3) → u_resolution (vec2)
// 3. iTime → u_time
// 4. gl_FragCoord → FlutterFragCoord()
// 5. mainImage(out vec4, in vec2) → void main() with out vec4 fragColor
// 6. 3./iResolution.y → 3.0/u_resolution.y
...
void sphere (vec4 hx, vec2 st, float sm, inout vec4 R)
{
    ...
    float   T = mod(u_time+h21(hx.zw*20.)*20.,20.)
        ,   d = .4* ((T < 3.) ? sin(T*.52) :
                    ((T < 6.) ? 1. :
                    ((T < 9.) ? sin((9.-T)*.52) :
                                0.)))
        ,   y = .4* ((T < 4.) ? sin((T-1.)*.52) :
                    ...
                    ((T < 8.5) ? sin((8.5-T)*.52) :
                    0.))) - .06
        ,   f = (.9 + noise(vec2(hx.x*50.+u_time*4.))*.3)
    ...
    f = noise(hx.xy*vec2(12,7)+vec2(0,u_time*-4.))*.25+.5;
...
void pixel (float hh, float sm, vec2 st, vec2 s, float n, vec4 R, inout vec4 C)
{
    ...
    float   b = ((4.-n)*2.2+.8)*.05
        ,   th = .05
        ,   T = mod(u_time+hh*20.,20.)
        ,   d = ((T < 3.) ? sin((T)*.52) :
                        ((T < 6.5) ? 1. :
                        ((T < 9.5) ? sin((9.5-T)*.52) :
                                0.)))
        ,   f =  min(
                (pow(noise(id*hh*n+u_time*(.75+h21(id)*.15)*1.),8.)*2.
                + (noise(id*.2 + u_time*(.5+hh*n)*.5)-.1))
...
void tile(vec2 uv, inout vec4 C)
{
    ...
    float n  = sqr.z
        , sm = 3.0/u_resolution.y   // ← iResolution.y → u_resolution.y
        , hh = h21(hx.zw*20.);
...
void main()
{
    vec2 rz = u_resolution.xy;
    vec2 uv = (FlutterFragCoord().xy + FlutterFragCoord().xy - rz) / -rz.y;

    // En portrait (móvil) el Y queda invertido respecto a Shadertoy
    // porque Shadertoy usa origen abajo-izquierda y Flutter arriba-izquierda
    uv.y = -uv.y;

    uv *= .8+sin(u_time*.3)*.25;
    uv -= uv * pow(length(uv), 2.5-sin(u_time*.3)*.5)*.025 +
        vec2(u_time*.2, cos(u_time*.2));
